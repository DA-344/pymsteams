...
class AppTeamManager extends BaseManager_1.default {
     ...
     */
    async create(modID, perms) {
        const data = await this.discloudApp.rest.post(v2_1.Routes.appTeam(this.app.id), {
            body: {
                modID,
     ...
     * @returns Promise {@link ApiAppTeamManager}
     */
    async edit(modID, perms) {
        const data = await this.discloudApp.rest.put(v2_1.Routes.appTeam(this.app.id), {
            body: {
                modID,
                perms: new util_1.ModPermissionsBF(perms).toArray(),
            },
     ...
     */
    async delete(modID) {
        const data = await this.discloudApp.rest.delete(v2_1.Routes.appTeam(this.app.id, modID));
        return data;
    }
    /**
     ...
     */
    async fetch() {
        const data = await this.discloudApp.rest.get(v2_1.Routes.appTeam(this.app.id));
        return data.team;
    }
}
