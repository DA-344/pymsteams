"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const v2_1 = require("@discloudapp/api-types/v2");
const BaseManager_1 = __importDefault(require("./BaseManager"));
/**
 * Manager for APT on your application on Discloud
 */
class AppAptManager extends BaseManager_1.default {
    app;
    constructor(discloudApp, app) {
        super(discloudApp);
        this.app = app;
    }
    /**
     * Install APTs for your application on Discloud
     *
     * @param apt - One or more APTs to install. See {@link AptString}.
     * @returns Promise {@link RESTPutApiAppAptResult}
     */
    async install(apt) {
        if (Array.isArray(apt))
            apt = apt.join();
        const data = await this.discloudApp.rest.put(v2_1.Routes.appApt(this.app.id), {
            body: {
                apt,
            },
        });
        return data;
    }
    /**
     * Uninstall APTs for your application on Discloud
     *
     * @param apt - One or more APTs to uninstall. See {@link AptString}.
     * @returns Promise {@link RESTDeleteApiAppAptResult}
     */
    async uninstall(apt) {
        if (Array.isArray(apt))
            apt = apt.join();
        const data = await this.discloudApp.rest.delete(v2_1.Routes.appApt(this.app.id), {
            body: {
                apt,
            },
        });
        return data;
    }
}
exports.default = AppAptManager;
