...
class default_1 extends Command_1.default {
    ...
    async run(task, item = {}) {
        ...
        item = extension_1.default.teamAppTree.children.get(item.appId);
        if (!item?.appId)
            return;
        if (!item.children.size)
            return extension_1.default.teamAppTree.fetch();
        const res = await (0, util_1.requester)(discloud_app_1.Routes.teamStatus(item.appId));
        if (!res.apps)
            return;
